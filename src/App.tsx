...
export default function App() {
  ...
  const exportToExcel = () => {
    ...
    setLoading(true);
    setMessage({ type: 'success', text: 'Gerando arquivo Excel...' });
    ...
    setTimeout(() => {
      try {
        ...
        saveAs(blob, "relatorio_ponto.xlsx");
        
        setMessage({ type: 'success', text: 'Relatório Excel gerado com sucesso!' });
      } catch (err) {
        console.error('Excel Export Error:', err);
        setMessage({ type: 'error', text: 'Erro ao gerar Excel. Tente abrir em uma nova aba se estiver no celular.' });
      } finally {
        setLoading(false);
      }
    }, 100);
  ...
  const exportToPDF = () => {
    if (reportRecords.length === 0) return;
    setLoading(true);
    setMessage({ type: 'success', text: 'Gerando arquivo PDF...' });
    ...
    setTimeout(() => {
      try {
        ...
        setMessage({ type: 'error', text: 'Erro ao gerar PDF. Tente abrir em uma nova aba se estiver no celular.' });
      } finally {
        setLoading(false);
      }
    }, 100);
